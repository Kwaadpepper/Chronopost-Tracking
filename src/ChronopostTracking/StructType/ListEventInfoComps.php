...
class ListEventInfoComps extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - nillable: true
     * @var \ChronopostTracking\StructType\EventInfoComp[]
     */
     ...
     * @param \ChronopostTracking\StructType\EventInfoComp[] $events
     */
     ...
     * @param string $skybillNumber
     * @return \ChronopostTracking\StructType\ListEventInfoComps
     */
    ...
    public static function validateEventsForArrayConstraintFromSetEvents(?array $values = []): string
    {
        ...
        foreach ($values as $listEventInfoCompsEventsItem) {
            ...
            if (!$listEventInfoCompsEventsItem instanceof \ChronopostTracking\StructType\EventInfoComp) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The events property can only contain items of type \ChronopostTracking\StructType\EventInfoComp, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \ChronopostTracking\StructType\EventInfoComp $item
     * @return \ChronopostTracking\StructType\ListEventInfoComps
     */
    public function addToEvents(\ChronopostTracking\StructType\EventInfoComp $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \ChronopostTracking\StructType\EventInfoComp) {
            throw new InvalidArgumentException(sprintf('The events property can only contain items of type \ChronopostTracking\StructType\EventInfoComp, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->events[] = $item;
