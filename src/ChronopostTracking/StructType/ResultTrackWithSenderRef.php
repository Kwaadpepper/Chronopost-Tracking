...
class ResultTrackWithSenderRef extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - nillable: true
     * @var \ChronopostTracking\StructType\ListEvents[]
     */
     ...
     * @param \ChronopostTracking\StructType\ListEvents[] $listParcel
     */
     ...
     * Set errorCode value
     * @param int $errorCode
     * @return \ChronopostTracking\StructType\ResultTrackWithSenderRef
     */
    ...
    public static function validateListParcelForArrayConstraintFromSetListParcel(?array $values = []): string
    {
        ...
        foreach ($values as $resultTrackWithSenderRefListParcelItem) {
            ...
            if (!$resultTrackWithSenderRefListParcelItem instanceof \ChronopostTracking\StructType\ListEvents) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The listParcel property can only contain items of type \ChronopostTracking\StructType\ListEvents, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \ChronopostTracking\StructType\ListEvents $item
     * @return \ChronopostTracking\StructType\ResultTrackWithSenderRef
     */
    public function addToListParcel(\ChronopostTracking\StructType\ListEvents $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \ChronopostTracking\StructType\ListEvents) {
            throw new InvalidArgumentException(sprintf('The listParcel property can only contain items of type \ChronopostTracking\StructType\ListEvents, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
