...
class EventInfoComp extends Event
{
    ...
    public static function validateInfoCompListForArrayConstraintsFromSetInfoCompList(?array $values = []): string
    {
        ...
        foreach ($values as $eventInfoCompInfoCompListItem) {
            ...
            if (!$eventInfoCompInfoCompListItem instanceof \StructType\InfoComp) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The infoCompList property can only contain items of type \StructType\InfoComp, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \StructType\InfoComp $item
     * @return \StructType\EventInfoComp
     */
    public function addToInfoCompList(\StructType\InfoComp $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\InfoComp) {
            throw new InvalidArgumentException(sprintf('The infoCompList property can only contain items of type \StructType\InfoComp, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
