...
class ListEventInfoComps extends AbstractStructBase
{
     ...
     * @param \StructType\EventInfoComp[] $events
     ...
     * @return \StructType\EventInfoComp[]
     */
    ...
    public static function validateEventsForArrayConstraintsFromSetEvents(?array $values = []): string
    {
        ...
        foreach ($values as $listEventInfoCompsEventsItem) {
            ...
            if (!$listEventInfoCompsEventsItem instanceof \StructType\EventInfoComp) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The events property can only contain items of type \StructType\EventInfoComp, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \StructType\EventInfoComp[] $events
     * @return \StructType\ListEventInfoComps
     */
     ...
     * @throws InvalidArgumentException
     * @param \StructType\EventInfoComp $item
     * @return \StructType\ListEventInfoComps
     */
    public function addToEvents(\StructType\EventInfoComp $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\EventInfoComp) {
            throw new InvalidArgumentException(sprintf('The events property can only contain items of type \StructType\EventInfoComp, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->events[] = $item;
