...
class ResultTrackSearch extends AbstractStructBase
{
     ...
     * @param \StructType\InfosPOD[] $listInfosPOD
     */
     ...
     * Set errorCode value
     * @param int $errorCode
     * @return \StructType\ResultTrackSearch
     */
    ...
    public static function validateListInfosPODForArrayConstraintsFromSetListInfosPOD(?array $values = []): string
    {
        ...
        foreach ($values as $resultTrackSearchListInfosPODItem) {
            ...
            if (!$resultTrackSearchListInfosPODItem instanceof \StructType\InfosPOD) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The listInfosPOD property can only contain items of type \StructType\InfosPOD, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \StructType\InfosPOD $item
     * @return \StructType\ResultTrackSearch
     */
    public function addToListInfosPOD(\StructType\InfosPOD $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\InfosPOD) {
            throw new InvalidArgumentException(sprintf('The listInfosPOD property can only contain items of type \StructType\InfosPOD, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
